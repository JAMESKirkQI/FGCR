...
class MaskedAutoencoderPVT(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3, stride=16,
                 ...
                 vis_mask_ratio=0.25, views=1, num_classes=1000):
        super().__init__()
...
def mae_pvt_small_256_dec128d2b(**kwargs):
...
mae_pvt_small = mae_pvt_small_256_dec128d2b  # decoder: 128 dim, 2 blocks
...
if __name__ == '__main__':
    ...
    model = mae_pvt_small(vis_mask_ratio=0.25, views=args.views).to("cuda")

    parameters = filter(lambda p: p.requires_grad, model.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1e6
    print('Trainable Parameters: %.3fM' % parameters)

    loss_rec, loss_hog, sim_loss, predicted_class = model(img, mask, args)
