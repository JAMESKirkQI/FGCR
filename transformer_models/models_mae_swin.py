...
from util.mask_transform import MultiRandomMaskingGenerator
...
class MaskedAutoencoderSwin(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3, stride=16,
        ...
        self.img_size = img_size
        expand_channel = 2
        self.expand_pixel = nn.PixelShuffle(expand_channel)
        self.decoder_embed_dim = decoder_embed_dim
        self.decoder_embed = nn.Linear(encoder_out_dim, 4 * decoder_embed_dim, bias=True)
        self.decoder_expand = nn.PixelShuffle(expand_channel)
    ...
    def forward_encoder(self, x, mask):
        ...
        mask = mask.repeat_interleave(scale, 1).repeat_interleave(scale, 2).unsqueeze(
            1).contiguous().bool()  # (N, 1, H, W)
        ...
        if hasattr(self, 'vis_mask_token'):
            ...
            vis_mask_token = vis_mask_token.reshape(N, self.embed_h, self.embed_w, self.embed_dim).permute(0, 3, 1,
                                                                                                           2)  # N C H W
    ...
    def forward_decoder(self, x, mask):
        ...
        for blk in self.decoder_blocks:
            x = blk(x)
        max_info = self.decoder_norm(x)

        # predictor projection
        x = self.decoder_pred(max_info)

        return x, pos_mask.shape[1], max_info
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        """
        imgs = self.patchify(imgs, self.stride)
        N, _, D = imgs.shape
        target = imgs[mask].reshape(N, -1, D)
    ...
    def forward(self, imgs, mask, args):
        # embed patches
        ...
        for blk in self.blocks:
            x, pos_hw = blk(x, pos_hw)
        latent_vanilla = x.mean(dim=1)  # global pool
        latent_vanilla = self.fc_norm(latent_vanilla)
        predicted_class = self.head(latent_vanilla)
        if len(mask.shape) == 2:
            mask = mask.reshape(-1, self.views, mask.shape[-1])
        max_info_list = []
        if args.lambda_weight < 1:
            loss_rec = 0
            for i in range(self.views):
                latent = self.forward_encoder(x_patches, mask[:, i])  # returned mask may change
                pred, mask_num, max_info = self.forward_decoder(latent, mask[:, i])  # [N, L, p*p*3]
                loss_rec += self.forward_loss(imgs, pred[:, -mask_num:], mask[:, i])
                max_info_list.append(max_info)
            loss_rec /= self.views
        else:
            loss_rec = 0
        ...
        # consistency_loss
        if args.sim_beta > 0:
            max_info_list = torch.stack(max_info_list)
            cls_rec_avg = max_info_list.clone().detach().mean(dim=0)
            cls_rec_avg = (cls_rec_avg - cls_rec_avg.mean(dim=0)) / cls_rec_avg.std(dim=0)
            ct_loss = 0
            for z_1 in max_info_list:
                z_1 = (z_1 - z_1.mean(dim=0)) / z_1.std(dim=0)
                c = z_1.mean(dim=0).T @ cls_rec_avg.mean(dim=0)
                on_diag = torch.diagonal(c).add_(-1).pow_(2).sum()
                off_diag = off_diagonal(c).add_(1).pow_(2).sum()
                ct_loss += (on_diag + 0.005 * off_diag)
            sim_loss = ct_loss / args.views
        else:
            sim_loss = 0
        return loss_rec, sim_loss, predicted_class


def mae_swin_tiny_256_window_8_dec128d2b(**kwargs):
...
def mae_swin_tiny_256_dec128d2b(**kwargs):
    model = MaskedAutoencoderSwin(
        img_size=256, patch_size=4, in_chans=3, stride=16,
        embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
...
mae_swin_tiny_256 = mae_swin_tiny_256_dec128d2b  # decoder: 128 dim, 2 blocks
mae_swin_tiny_256_w8 = mae_swin_tiny_256_window_8_dec128d2b  # decoder: 128 dim, 2 blocks

if __name__ == '__main__':
    img = torch.ones([2, 3, 256, 256]).to("cuda")
    ...
    args = ARGS()
    masked_position_generator = MultiRandomMaskingGenerator(
        16, 0.75, True, args.views
    )
    mask = torch.tensor(masked_position_generator(), dtype=torch.bool)
    # mask = torch.cat([mask, mask], dim=0).reshape(2, args.views, -1).to("cuda")
    mask = torch.cat([mask, mask], dim=0).to("cuda")

    print(args.views)
    print(args.mask_ratio)

    model = mae_swin_tiny_256_w8(vis_mask_ratio=0.25, views=args.views).to("cuda")
    # model = mae_swin_tiny_256_w8(vis_mask_ratio=0.25, views=args.views)

    parameters = filter(lambda p: p.requires_grad, model.parameters())
    parameters = sum([np.prod(p.size()) for p in parameters]) / 1e6
    print('Trainable Parameters: %.3fM' % parameters)

    # loss_rec, loss_hog, mmd_loss, sim_loss, predicted_class = model(img, args)
    start_time = time.time()
    for i in range(3):
        loss_rec, sim_loss, predicted_class = model(img, mask, args)
    print(f"cost time{time.time() - start_time}")
    # x = model.forward_test(img)
    print("loss_rec {}, sim_loss {} :".format(loss_rec, sim_loss))  # [B, num_classes]
    # print("Shape of predicted_class :", x.shape)  # [B, num_classes]
